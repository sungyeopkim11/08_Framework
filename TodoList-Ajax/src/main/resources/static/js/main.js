...
function selectTodoList(){
  ...
  .then(result => {
    ...
    for(let todo of todoList){
      ...
      todoComplete.innerText = (todo.todoComplete == 1) ? '했음' : '안했음';

      // 4) regDate에 들어갈 td요소 생성
...
function selectTodo(url){
  ...
  .then(todo => {
    ...
    // 완료 여부 - 0 == X, 1 == O
    popupComplete.innerText = todo.todoComplete == 1 ? '했음' : '안했음';
...
changeComplete.addEventListener("click", () => {
  ...
  .then(result => {
    ...
    todoComplete.innerText = (todoComplete.innerText == '했음') ? '안했음' : '했음';
...
updateView.addEventListener("click", () => {
  ...
  // 상세 조회 제목/내용
  const todoTitle = document.querySelector("#popupTodoTitle").innerHTML;
