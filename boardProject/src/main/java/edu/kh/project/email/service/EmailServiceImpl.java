...
public class EmailServiceImpl implements EmailService {
	...
	public int sendEmail(String htmlName, String email) {
		...
		try {
			...
			case "findPw" :
				emailTitle = "[boardProject] 비밀번호 찾기 인증 번호 입니다.";
				break;
			}
			...
			// 매개 변수1 : 이메일 내용
			// 매개 변수2 : HTML 코드 해석 여부 지정(true == 해석 0)
			
			// 지정된 HTML 파일에 authKey가 첨부된 후
			// HTML 코드 전체가 하나의 String으로 변환되서 반환 받은 후
			// 변환된 String을 메일 내용으로 세팅
			
			helper.addInline("logo", new ClassPathResource("static/images/email.jpg"));
  ...
  //HTML 파일을 읽어와 String으로 변환 (타임리프 적용)
	...
	public boolean checkAuthKey(Map<String, String> map) {
		...
		// 2) Redis에 같은 key값이 있다면 value를 얻어와
		//    입력 받은 인증 번호와 비교
