 ...
 ex) 전화
  ...
  - ws:// 프로토콜 요청(http가 아님!!)
    ...
    --> SessionHandShakeInterceptor가
      HTTP Session을 가로채서 Websoket에서 사용 가능한 형태로 변환

    -> 어떤 요청 주소가 Websoket 요청인지 지정
*/
// 채팅에 사용될 SockJS 객체를 저장할 변수
let chattingSock;

// 로그인이 되어 있을 경우 
if(notificationLoginCheck){ // common.html에 저장된 전역변수

  // 서버로 ws://chattingSock 요청
  // -> 요청으로 처리하는 WebSoketHandler와 연결
  //  --> WebSoketHandler에 연결된 회원의 정보를 모아두게 된다.
