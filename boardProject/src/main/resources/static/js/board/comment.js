...
const selectCommentList = () => {
  ...
  .then(html => {
    ...
    /* [주의 사항] */
    // innerHTML로 새로 만들어지 요소에는
    // 이벤트 리스너가 추가되어 있지 않기 때문에
    // 답급, 수정, 삭제 등이 동작하지 않는다!!
...
const insertComment = (parentCommentNo) => {
  ...
  fetch("/comment", {
    ...
    body : JSON.stringify(data) // JS객체 -> JSON (문자열)
  })
  .then(response => {
    if(response.ok) return response.text();
