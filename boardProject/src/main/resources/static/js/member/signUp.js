...
memberEmail.addEventListener("input", e => {
  ...
  if( regEx.test(inputEmail) === false ){ // 입력값이 이메일 형식이 아닐경우
  ...
  .then(count => {
    // 매개 변수 count : 첫 번째 then에서 return 된 값이 저장된 변수
    if(count == 1){ // 중복인 경우
...
memberNickname.addEventListener("input", () => {
  ...
  if(inputNickname.length === 0){
    ...
    // #nickMemssage에 색상 관련 클래스를 모두 제거
  ...
  if( regEx.test(inputNickname) === false ){ // 잘못된 값이 입력됐을 경우
  ...
  .then(count => {
    ...
    checkObj.memberNickname = true; // 유효한 이메일임을 기록
  })
...
const telMessageObj = {};
telMessageObj.normal = "전화번호를 입력하세요.(-제외)";
...
pwMessageObj.invaild = "유효하지 않은 비밀번호 형식 입니다";
pwMessageObj.vaild = "유효한 비밀번호 형식 입니다";
...
const initMin  = 4; // 초기 값 5분에 1초 감소된 후 분
const initSec  = 59; // 초기 값 5분에 1초 감소된 후 초
...
/* 전달 받은 숫자가 10 미만(한 자리수)인 경우 앞에 0을 붙여서 반환하는 함수 */
  ...
  for(let key in checkObj){ 
    ...
    if( checkObj[key] === false ){ // 유효하지 않은 경우
      ...
      switch(key){
        ...
        case "memberAuthKey"   : str = "이메일이 인증되지 않았습니다"; break;
      }
