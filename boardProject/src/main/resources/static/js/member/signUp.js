...
memberEmail.addEventListener("input", e => {
  ...
  if( regEx.test(inputEmail) === false ){ // 입력값이 이메일 형식이 아닐경우
    ...
    checkbobj.memberEmail = false; // 유효하지 않다고 체크
    return;
  }
  ...
  .then(count => {
    ...
    checkbobj.memberEmail = true; // 유효한 이메일임을 기록
  })
...
memberNickname.addEventListener("input", () => {
  ...
  if(inputNickname.length === 0){
    ...
    checkbobj.memberNickname = false;
  ...
  .then(count => {
    ...
    checkbobj.memberNickname = true; // 유효한 이메일임을 기록
  })
...
memberPwConfirm.addEventListener("input", () => {
  ...
  if(checkbobj.memberPw === true){
    checkPw();
    return;
  }

  // 비밀번호 input에 작성된 값이 유효하지 않은 경우
  checkbobj.memberPwConfirm = false;
