...
memberNickname?.addEventListener("input", () => {
  ...
  // get방식 요청 (쿼리 스트링으로 파라미터 전달)
  fetch("/myPage/checkNickname?input=" + inputValue)
  .then(response => {
    if(response.ok) {
      return response.text(); // 응답 결과를  text형태로 변경
    }
...
updateInfo?.addEventListener("submit", e => {
  ...
  for(let key in checkObj) {
    ...
    if(checkObj[key] === false){
      ...
      alert(str); // OOO이 유효하지 않습니다
      e.preventDefault(); // form 제출 막기

      document.getElementById(key).focus(); // 포커스 맞추기
      return;
    }
  }
...
if(imageInput != null){ // 프로필 변경 화면인 경우
 ...
 const updatePreview = (file) => {
  ...
  deleteImage.addEventListener("click", () => {
    ...
    // input 태그와 마지막 선택된 파일을 저장하는 lastVaildFile에 
    // 저장된 값을 모두 삭제
    imageInput.value = '';
    lastVaildFile = null;
