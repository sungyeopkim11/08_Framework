...
/* 단방향 통신 (ex : 무전기)
 ...
 1) 클라이언트 서버에 연결
    -> 클라이언트가 서버로부터 데이터 받기 위한
       대기상태에 돌입 (EventSource 객체 이용)
...
/* SSE 연결하는 함수
  -> 연결을 요청힌 클라이언트가 서버로부터 데이터가 전달될 떄 까지 대기 상태
     (비동기)
*/
const connectSse = () => {
  ...
  /* 서버로부터 전달 받은 메시지가 왔을 경우(전달 받은 경우) */
...
/* 읽지 않은 알림 개수 조회 및 알림 유뮤 표시 여부 변경 */
const notReadCheck = () => {
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  */
  ...
  if(cn != null){
    const targetId = "c" + cn; // "c1000", "c1234" 형태로 변환
